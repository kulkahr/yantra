...
|---|---|---|---|
| U1 | Whether the scale enforces an app-side *challenge secret* beyond `verificationCode ⊕ MAC` (i.e., auth is really just MAC-based) | If a secret existed, Firefly could connect but fail auth | Decompiled auth path shows no secret — response echoes the scale's own challenge back. **Experiment (E1):** macOS Swift host attempts full pair handshake; success definitively closes this. Expected: success. |
| U2 | Exact live-weight stream frame type (raw `A621` short frames vs `0x00E9` records) on this firmware | Minor — affects live UI latency only | SRD-003 already handles both paths (final record + stream). **Experiment (E2):** log all `A621` notifies during a weigh-in; classify frames. |
